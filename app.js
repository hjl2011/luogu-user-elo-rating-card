...
(async function () {
    ...
    svg_file.write(`<svg width="350" height="${190 + (config['enable-show-history-maxvalue'] ? 30 : 0)}" version="1.1" xmlns="http://www.w3.org/2000/svg" style="cursor:default;user-select:none;">`);
    ...
    svg_file.write(`<text x="100" y="${175 + (config['enable-show-history-maxvalue'] ? 30 : 0)}" style="fill: #888;font-size: 14px;">${lang[config.lang].card_generation_time}${Date_Format(new Date())}</text>`);
    svg_file.write('</svg>');
